...
def generate_text():
        ...
        if new_data:
            ...
            for data in yomi_segments_data:
                ...
                if not (len(yomi_slice_raw) == len(mapping_slice_raw) == len(word_map_slice)):
                    logging.warning(f"Generate2 (Fallback): Mismatch raw slice lengths. Skipping segment.")
                    continue
    ...
    for data in yomi_segments_data:
        ...
        if not (len(yomi_slice_raw) == len(mapping_slice_raw) == len(word_map_slice)):
            logging.warning(f"Generate2 (Cache): Mismatch raw slice lengths. Skipping segment.")
            continue

...
// DOM要素 (initGame で取得)
let textBox, yomiBox, renda, remainingTime, startBox, resultBox, centerBox, selectBox, endBox, start_text, jikan, possible_text; // endBox を追加
...
endBox = document.getElementById('end-box'); // ★ 修正: 取得する
...
endBox.style.display = 'none'; // ★ 追加: 終了画面も非表示
...
remainingTime.style.display = ''; // ★ 修正: displayスタイルをリセット
...
start = 1; // (true の方が望ましいが、元のコードを尊重)

// 1秒待ってからゲーム画面へ

// ★ 修正: スタイル設定を setTimeout の中に移動
...
// ★ 修正: white-space の設定をここに移動
// HTMLに id="odai-box" があり、
// それが textBox や yomiBox を囲んでいることを想定
const odai_box = document.getElementById('odai-box');
if (odai_box) { 
const items = odai_box.querySelectorAll('*'); // 元のロジックを維持
items.forEach(el => {
el.style.whiteSpace = (currentCourseConfig.special === true) ? 'normal' : 'nowrap';
});
} else {
// フォールバック（odai-box がない場合、主要要素に直接適用）
const isSpecial = currentCourseConfig.special === true;
const whiteSpaceStyle = isSpecial ? 'normal' : 'nowrap';
if (textBox) textBox.style.whiteSpace = whiteSpaceStyle;
if (yomiBox) yomiBox.style.whiteSpace = whiteSpaceStyle;
if (possible_text) possible_text.style.whiteSpace = whiteSpaceStyle;
}
...
// タイマースタート
// ★ 修正: special でない (false) 時にタイマーを開始する
...
// ★ 修正: nokorijikan が null (specialモード) の場合はタイマー処理をしない
// (ただし、startGame の修正により、specialモードではこの関数は呼ばれないはず)
if (nokorijikan === null) {
if (secondsTimer) clearInterval(secondsTimer);
secondsTimer = null;
return;
...
// (if (currentCourseConfig.special !== true) { ... } のネストは不要)
// 合計皿数
document.getElementById('total_got_odai').textContent = `${i + 1} 皿`;
} else {
// (special === true の場合の完了時処理)
// (必要ならここに記述)
}
// 次の単語へ
setNextWord();
...
// KPS と 正確率の表示
let correct_keys_persent = 0;
if (correct_keys_count + incorrect_keys_count > 0) {
correct_keys_persent = parseFloat((correct_keys_count / (correct_keys_count + incorrect_keys_count)) * 100).toFixed(1);
}
...
if (addedTime > 0) {
if (nokorijikan !== null) { // special モードでない場合のみ時間を加算
...
*/
function endGame() {
start = false; // ★ 修正: start = null ではなく false

// ★ 修正: 終了時刻が記録されていない場合 (時間切れ以外での終了)
...
setTimeout(() => {
let total = 0;
if (currentCourseConfig.special !== true) { // ★ special モードでは金額計算しない
...
key_per_second = (correct_keys_count) / elapsedTimeSeconds; // ★ 修正: 正解キーのみ
}
}
...
const elapsedTimeSeconds = (end_time - start_time) / 1000;
if (elapsedTimeSeconds > 0) {
// ★ 修正: 元のコードに合わせて (correct + incorrect) / time
